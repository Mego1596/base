...
public class EliminarTabla1 extends AppCompatActivity implements Response.ErrorListener, Response.Listener<JSONObject> {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        codigoMateria = (EditText) findViewById(R.id.editDeleteMateria);
        request= Volley.newRequestQueue(getApplicationContext());
    }


    public void cargarWebServiceLocal(View v){
        if(codigoMateria.getText().toString().isEmpty()){
            Toast.makeText(getApplicationContext(),"Debes llenar todos los campos ",Toast.LENGTH_LONG).show();
        }else{
            String url ="http://192.168.1.113/ws_eliminar_materia.php?codmateria="+ codigoMateria.getText().toString();
            url=url.replace(" ","%20");
    ...
    public void cargarWebServiceExterno(View v){
        if(codigoMateria.getText().toString().isEmpty()){
            ...
            String url ="https://go14002.000webhostapp.com//ws_eliminar_materia.php?codmateria="+ codigoMateria.getText().toString();
            url=url.replace(" ","%20");
